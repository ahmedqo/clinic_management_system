...
@section('title', __('Update Event') . ' #' . $data->id)

@section('content')
    <div class="flex flex-col gap-4">
        <div class="w-full flex items-center justify-between gap-2">
            <h1 class="font-x-core text-2xl">{{ __('Update Event') }} #{{ $data->id }}</h1>
            <div
                ...
                <div class="container mx-auto lg:w-max p-4 lg:p-0">
                    <div
                        ...
                        <a href="{{ route('actions.events.delete', $data->id) }}"
        ...
        <div class="toremove bg-x-white rounded-x-core shadow-x-core p-4">
            <form id="form" action="{{ route('actions.events.update', $data->id) }}" method="POST"
                ...
                <div class="flex flex-col gap-px lg:col-span-3">
                    ...
                    <select x-select search id="start_time" name="start_time" placeholder="{{ __('Start Time') }}">
                        @foreach (Core::slot() as $slot)
                            <option value="{{ $slot }}" {{ $slot == $data->start_time ? 'selected' : '' }}>
                                {{ ucwords(__($slot)) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex flex-col gap-px lg:col-span-3">
                    ...
                    <select x-select search id="end_time" name="end_time" placeholder="{{ __('End Time') }}">
                        @foreach (Core::slot() as $slot)
                            <option value="{{ $slot }}" {{ $slot == $data->end_time ? 'selected' : '' }}>
                                {{ ucwords(__($slot)) }}
                            </option>
                        @endforeach
                    </select>
