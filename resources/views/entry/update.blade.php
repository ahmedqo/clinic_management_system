...
@section('title', __('Update Entry') . ' #' . $data->id)

@section('content')
    <div class="flex flex-col gap-4">
        <div class="w-full flex items-center justify-between gap-2">
            <h1 class="font-x-core text-2xl">{{ __('Update Entry') }} #{{ $data->id }}</h1>
            <div
                ...
                <div class="container mx-auto lg:w-max p-4 lg:p-0">
                    <div
                        ...
                        <a href="{{ route('actions.entries.delete', $data->id) }}"
        ...
        <div class="toremove bg-x-white rounded-x-core shadow-x-core p-4">
            <form id="form" action="{{ route('actions.entries.update', $data->id) }}" method="POST"
                ...
                <div class="flex flex-col gap-px lg:col-span-6">
                    <label for="content" class="text-x-black font-x-core text-sm">{{ __('Content') }}</label>
                    <textarea id="content" name="content" placeholder="{{ __('Content') }}" rows="8"
                        class="bg-x-light text-x-black border-x-shade focus-within:outline-x-prime p-2 text-base border rounded-md focus-within:outline focus-within:outline-2 focus-within:-outline-offset-2">{!! $data->content !!}</textarea>
                </div>
            </form>
        </div>
    </div>
