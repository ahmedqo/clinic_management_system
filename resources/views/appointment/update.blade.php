...
@section('title', __('Update Appointment') . ' #' . $data->id)

@section('content')
    <div class="flex flex-col gap-4">
        <div class="w-full flex items-center justify-between gap-2">
            <h1 class="font-x-core text-2xl">{{ __('Update Appointment') }} #{{ $data->id }}</h1>
            <div
                ...
                <div class="container mx-auto lg:w-max p-4 lg:p-0">
                    <div
                        ...
                        <a href="{{ route('actions.appointments.delete', $data->id) }}"
        ...
        <div class="toremove bg-x-white rounded-x-core shadow-x-core p-4">
            <form id="form" action="{{ route('actions.appointments.update', $data->id) }}" method="POST"
                ...
                <div class="flex flex-col gap-px lg:col-span-3">
                    ...
                    <select x-select search id="status" name="status" placeholder="{{ __('Status') }}">
                        @foreach (Core::status() as $status)
                ...
                <div class="flex flex-col gap-px lg:col-span-3">
                    ...
                    <input x-date id="date" type="date" name="date" placeholder="{{ __('Date') }}"
                        value="{{ $data->date }}" disabled-days="{{ Core::disabled() }}" />
                </div>
                <div class="flex flex-col gap-px lg:col-span-3">
                    ...
                    <select x-select search id="time" name="time" placeholder="{{ __('Time') }}">
                        @foreach (Core::timeSlot() as $slot)
                            <option value="{{ $slot }}" {{ $slot == $data->time ? 'selected' : '' }}>
                                {{ $slot }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex flex-col gap-px lg:col-span-6">
                    ...
                    <textarea id="note" name="note" placeholder="{{ __('Note') }}" rows="2"
                        class="bg-x-light text-x-black border-x-shade focus-within:outline-x-prime p-2 text-base border rounded-md focus-within:outline focus-within:outline-2 focus-within:-outline-offset-2">{!! $data->note !!}</textarea>
                </div>
            </form>
        </div>
    </div>
