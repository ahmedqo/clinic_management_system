...
@section('title', __('Invoice Summary') . ' #' . $data->id)

@section('content')
    <div class="flex flex-col gap-4">
        <div class="w-full flex items-center justify-between gap-2">
            <h1 class="font-x-core text-2xl">{{ __('Invoice Summary') }} #{{ $data->id }}</h1>
            <div
                ...
                <div class="container mx-auto lg:w-max p-4 lg:p-0">
                    <div
                        ...
                        <a href="{{ route('views.invoices.update', $data->id) }}"
        ...
        <div class="toremove bg-x-white rounded-x-core shadow-x-core p-4">
            <div class="w-full grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <table default x-table search>
                        ...
                        <tbody>
                            @foreach ($data->items() as $item)
                                <tr>
                                    <td>{{ $item->content }}</td>
                                    <td>{{ $item->cost }} {{ Core::system()->currency }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="flex flex-col gap-px">
                    <label class="text-x-black font-x-core text-sm">{{ __('Total') }}</label>
                    <div class="w-full bg-x-light text-x-black border-x-shade p-2 text-base border rounded-md">
                        {{ $data->items()->sum('cost') }}
                ...
                <div class="flex flex-col gap-px">
                    <label class="text-x-black font-x-core text-sm">{{ __('Payment') }}</label>
                    <div class="w-full bg-x-light text-x-black border-x-shade p-2 text-base border rounded-md">
                        {{ ucwords(__($data->payment)) }}
                    </div>
                </div>
                @if ($data->note)
    ...
    <section id="page" class="w-full hidden">
        ...
        <table class="w-full">
                ...
                <tr>
                    <td>
                        <div class="flex justify-between gap-4 py-1 items-center">
                            <div class="text-x-black text-lg flex-1">
            ...
            <tr class="border-x-shade border-t">
                <td>
                    <div class="flex justify-between gap-4 py-1 items-center">
                        <div class="text-x-black text-lg flex-1 font-x-core">
                            {{ __('Total') }}
                        </div>
                        <div class="text-x-black text-lg">
                            {{ $data->items()->sum('cost') }}
