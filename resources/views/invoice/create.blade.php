...
@section('content')
    <div class="flex flex-col gap-4">
        <div class="w-full flex items-center justify-between gap-2">
            <h1 class="font-x-core text-2xl">{{ __('Create Invoice') }}</h1>
            <div
        ...
        <div class="toremove bg-x-white rounded-x-core shadow-x-core p-4">
            <form id="form" action="{{ route('actions.invoices.create') }}" method="POST"
                ...
                <div class="flex flex-col gap-px lg:col-span-3">
                    <label for="payment" class="text-x-black font-x-core text-sm">{{ __('Payment') }}</label>
                    <select x-select search id="payment" name="payment" placeholder="{{ __('Payment') }}">
                        @foreach (Core::payment() as $type)
                            <option value="{{ $type }}">
                                {{ ucwords(__($type)) }}
                            </option>
                        @endforeach
                ...
                <div class="flex flex-col gap-px lg:col-span-6">
                    ...
                    <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-6 gap-4 lg:gap-[5px]">
                            ...
                            <table class="w-max min-w-full">
                                <thead>
                                    <tr>
                                        ...
                                        <td class="text-x-black text-sm font-x-core p-2">{{ __('Cost') }}</td>
                                ...
                                <tbody id="row_display">
                                    <tr class="border-x-shade border-t">
                                        <td class="p-2 text-x-black text-base">
                                            {{ __('Consulation') }}
                                            <input type="hidden" name="items_content[]" value="{{ __('Consulation') }}" />
                                        </td>
                                        <td class="p-2 text-x-black text-base">
                                            {{ Core::system()->cost }} {{ Core::system()->currency }}
                                            <input type="hidden" name="items_cost[]" value="{{ Core::system()->cost }}" />
                                        </td>
                                        <td class="p-2">
                                            <button type="button" onclick="invoiceItemRow.remove(event)"
                                                class="mx-auto px-3 py-1 flex items-center justify-center rounded-md text-x-white hover:text-x-black focus-within:text-x-black bg-red-400 hover:bg-red-300 focus-within:bg-red-300 outline-none">
                                ...
                                <tfoot>
                                    <tr class="border-x-shade border-t">
                                        ...
                                        <td class="text-x-black text-base font-x-core p-2">
                                            <span id="total">{{ Core::system()->cost }}</span>
                                            {{ Core::system()->currency }}
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
                        ...
                        <div class="flex flex-col gap-4 lg:gap-[5px] lg:flex-row lg:flex-wrap lg:col-span-6">
                            ...
                            <div class="lg:flex-1">
                                <input id="row_cost" placeholder="{{ __('Cost') }}"
                                    class="bg-x-light w-full text-x-black border-x-shade focus-within:outline-x-prime p-2 text-base border rounded-md focus-within:outline focus-within:outline-2 focus-within:-outline-offset-2" />
                            </div>
