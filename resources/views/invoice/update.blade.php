...
@section('title', __('Update Invoice') . ' #' . $data->id)

@section('content')
    <div class="flex flex-col gap-4">
        <div class="w-full flex items-center justify-between gap-2">
            <h1 class="font-x-core text-2xl">{{ __('Update Invoice') }} #{{ $data->id }}</h1>
            <div
                ...
                <div class="container mx-auto lg:w-max p-4 lg:p-0">
                    <div
                        ...
                        <a href="{{ route('actions.invoices.delete', $data->id) }}"
        ...
        <div class="toremove bg-x-white rounded-x-core shadow-x-core p-4">
            <form id="form" action="{{ route('actions.invoices.update', $data->id) }}" method="POST"
                ...
                <div class="flex flex-col gap-px lg:col-span-3">
                    ...
                    <select x-select search id="payment" name="payment" placeholder="{{ __('Payment') }}">
                        @foreach (Core::payment() as $type)
                            <option value="{{ $type }}" {{ $type == $data->payment ? 'selected' : '' }}>
                                {{ ucwords(__($type)) }}
                            </option>
                        @endforeach
                    </select>
                                ...
                                @foreach ($data->items() as $item)
                                    <tr class="border-x-shade border-t">
                                        <td class="p-2 text-x-black text-base">
                                            {{ $item->content }}
                                            <input type="hidden" name="items_content[]" value="{{ $item->content }}"
                                                disabled />
                                        </td>
                                        <td class="p-2 text-x-black text-base">
                                            {{ $item->cost }} {{ Core::system()->currency }}
                                            <input type="hidden" name="items_cost[]" value="{{ $item->cost }}"
                                                disabled />
                                        </td>
                                        <td class="p-2">
                            ...
                            <tfoot>
                                <tr class="border-x-shade border-t">
                                    ...
                                    <td class="text-x-black text-base font-x-core p-2">
                                        <span id="total">{{ $data->items()->sum('cost') }}</span>
...
@section('scripts')
    <script>
        ...
        const [addRow, removeRow] = invoiceItemRow({
            display: "#row_display",
            content: "#row_content",
            cost: "#row_cost",
            total: "#total",
